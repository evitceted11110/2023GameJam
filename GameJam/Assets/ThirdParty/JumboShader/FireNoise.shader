Shader "Yile/Fire Noise"
{
    Properties
    {
        ...
        _ScrollX(" Scroll Noise X", Range(-30,30)) = 1
        _ScrollY(" Scroll Noise Y", Range(-30,30)) = 1
...
#include "UnityCG.cginc"
			...
			v2f vert(appdata v)
			{
				...
				float worldX = mul(unity_ObjectToWorld, v.vertex).x;

				o.uv = TRANSFORM_TEX(fixed2(worldX, v.uv.y), _NoiseTex);
			...
			{
				float4 gradientMain = lerp(_ColorB, _ColorA, (i.uv3.y + _Offset));
				float4 gradientTint = lerp(_TintB, _TintA, (i.uv3.y + _Offset));
				float4 gradientBlend = lerp(float4(2,2,2,2), float4(0, 0, 0, 0), (i.uv3.y + _Height));
 
				fixed4 uvdistort = tex2D(_DistortTex, i.uv3) * _Distort;
				fixed4 noise = tex2D(_NoiseTex,fixed2((i.uv.x + _Time.x* _ScrollX) + uvdistort.g  ,(i.uv.y + _Time.x* _ScrollY) + uvdistort.r));
				fixed4 shapetex = tex2D(_ShapeTex, i.uv2);
#ifdef SHAPE_ON
				noise = 1 - (noise * _Height + (1 - (shapetex * _Hard)));
#else  
				noise += gradientBlend;
#endif
#ifdef SHAPEX_ON
