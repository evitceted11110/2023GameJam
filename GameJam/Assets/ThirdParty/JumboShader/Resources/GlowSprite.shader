Shader "Jumbo/Effect/GlowSprite"
{
	Properties
	{
	...
	_Color("Tint", Color) = (1,1,1,1)
		...
		_ShineWidth("ShineWidth", Range(2,50)) = 0
...
#include "UnityCG.cginc"
	...
	fixed4 SampleSpriteTexture(float2 uv)
	{
		...
		if (currentDistanceProjection > lowLevel && currentDistanceProjection < highLevel) {
			float whitePower = 1 - (abs(currentDistanceProjection - _ShineLocation) / _ShineWidth) + _ShineWidth;
